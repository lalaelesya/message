...
async function generateLetter() {
  ...
  // Unique, collision-proof letter ID — short timestamp + random suffix
  const letterId = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  payload.letterId = letterId;

  state.letterData = payload;

  // Store full letter data in localStorage keyed by its unique ID.
  // The shared URL contains ONLY the short ID — no data in the URL at all.
  try {
    persistLetter(letterId, payload);
  } catch(e) {
    alert('Storage failed — letter may be too large (try fewer or smaller photos).');
    return;
  }

  // Clean, short shareable URL
  const baseUrl   = location.href.split('?')[0].split('#')[0];
  state.shareLink = baseUrl + '?letter=' + letterId;
...
//  The URL only contains a short ID (?letter=xyz).
//  All letter data lives in localStorage on the creator's device.
//  Recipients on the SAME device/browser open the letter instantly.
//  Different device = "Letter Not Found" with a clear explanation.
// ══════════════════════════════════════════════
window.addEventListener('load', () => {
  const params   = new URLSearchParams(location.search);
  const letterId = params.get('letter');

  // No ?letter= param = normal landing page visit, do nothing
  if (!letterId) return;

  // Look up this letter ID in localStorage
  const letterData = loadLetterFromStorage(letterId);
...
function showLetterNotFound() {
  ...
  content.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;
      min-height:80vh;text-align:center;padding:2rem;gap:0;">
      ...
      <h2 style="font-family:'DM Serif Display',serif;font-size:2rem;
        ...
        margin-bottom:0.8rem;">Letter Not Found</h2>
      <p style="color:rgba(255,255,255,0.45);font-style:italic;max-width:440px;line-height:1.9;
        margin-bottom:1.5rem;font-family:'Cormorant Garamond',serif;font-size:1.05rem;">
        Love letters are saved on the creator's device.<br>
        To share, open this link on the <strong style="color:rgba(255,255,255,0.65);">same browser</strong>
        where the letter was created &mdash; or use the <strong style="color:rgba(255,255,255,0.65);">Preview Letter</strong>
        button right after generating it.
      </p>
